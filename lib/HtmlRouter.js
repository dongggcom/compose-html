...
function Router (req) {
  ...
  const getFilepathWithDefault = (filepath) => filepath === this.rootpath ? filepath  + 'index.html' : filepath
  ...
  this.store = {
    ...
    loadAll: async (root = rootPath) => {
      ...
      return memorizeDirReadList(root).then(({ list, dirs }) => {
        ...
        this.store.set(root, { list: copy, dirs })
    ...
    load: async (abPath) => {
      ...
      if( data && _.isBuffer(data) ){
        ...
        const filepath = getFilepathWithDefault(abPath)
...
Router.prototype.route = async function (path) {
  const load =  await this.store.load(path)
  return load;
}

// 根据路径渲染html，当未传递path时，渲染当前请求的文件
Router.prototype.render = function(path){
  const loadPath = path || (this.filepath === this.rootpath ? this.filepath + 'index.html' : this.filepath)

  // 预加载全部页面
  return this.routeAll(this.rootpath)
    .then(() => this.route(loadPath))
